...
export const searchFilter = async (condition, status) => {
  ...
  switch (status) {
    case "active":
      data.forEach((auctionItem) => {
        if (
          auctionItem.Titel.includes(condition) &&
          dateConverter(auctionItem.SlutDatum) > currentDate
        ) {
      ...
      filteredList.sort(
        (auction1, auction2) =>
          dateConverter(auction1.SlutDatum) - dateConverter(auction2.SlutDatum)
      );
      break;
...
export const updateAuction = async (formData) => {
  const response = await api.put("Auktion/2460", formData);
  return response;
};
export const deletAuction = async (id) => {
  await api.delete(`Auktion/2460/${id}`);
};
